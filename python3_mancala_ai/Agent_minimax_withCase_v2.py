...
def case_cap(player_b,oppo_b):#capture opponent(player have chance to cap oppo, return how many available)
    ...
    for i in range(6):
        if(player_b[i]%13+i<6 and (player_b[player_b[i]%13+i]==0 or player_b[i]%13==0) and oppo_b[5-i]!=0):
            pos_num +=1
    return pos_num
##----------other calculation functions for score calculating
    ...
    if(MAXSTEP==0):                                           #when we reach the end of the prediction 
        ...
        if(isPlayer):                                     #return the difference of score the bigger the better for us
            if(p_man>24):
                ...
                score += case_cap(p_board,op_board)#how many chance to cap enemy-opposite
                score += case_free(p_board)*2#-case_free(op_board)
                score += diffMancala(p_man,op_man)#*player_id#mancala difference
                score += sumPB#*(player_id)#+player_id-1)#-sumOB#board
                #score += weightSum(p_board)/(3 +(0.5 if player_id==1 else 0))#the weighted sum is helping in certain situation
                score += fillNum(p_board)*2#filled hole number(player's)
#                score -= op_man/6#score deduction if enemy close to win
#                score = (score*0.65) if (score>0 and op_man==24) else score
#                score = (score*0.95) if (score>0 and op_man==23) else score
                return score
            ...
            if(p_man>24):
                ...
                score += case_cap(op_board,p_board)#how many chance to cap enemy-opposite
                score += case_free(op_board)*2#-case_free(p_board)
                score += diffMancala(op_man,p_man)#*player_id#mancala difference
                score += sumOB#*(player_id)#+player_id-1)#-sumPB #board
                #score += weightSum(op_board)/(3 +(0.5 if player_id==1 else 0))#the weighted sum is helping in certain situation
                score += fillNum(op_board)*2#filled hole number
#                score -= p_man/6#score deduction if enemy close to win
#                score = (score*0.65) if (score>0 and p_man==24) else score
#                score = (score*0.95) if (score>0 and p_man==23) else score
                return score
    ##########################################################################
    ####evaluation############################################################
    pick=0
