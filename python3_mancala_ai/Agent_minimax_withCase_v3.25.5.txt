...
def case_cap(player_b,oppo_b):#capture opponent(player have chance to cap oppo, return how many available)
    ...
    for i in range(6):
        if(player_b[i]!=0 and player_b[i]%13+i<6 and (player_b[player_b[i]%13+i]==0 or player_b[i]%13==0) and oppo_b[5-(player_b[i]%13+i)]!=0):
            pos_num +=1
    return pos_num
def case_cap_biggest(player_b,oppo_b):#capture opponent(player have chance to cap oppo, return largest capture)
    seed_num=0
    for i in range(6):
        target_pos = player_b[i]%13+i #the end position if player go with this slot
        if(player_b[i]!=0 and target_pos<6 and (player_b[target_pos]==0 or target_pos==i) and oppo_b[5-(target_pos)]!=0):
            seed_num = max(seed_num,oppo_b[5-(target_pos)])
    return seed_num
...
def fillNum123(player_b):#the holes filled in 123 holes
    ...
    for i in range(3):
        if(player_b[i]!=0):
            numHoleFill+=1
    return numHoleFill
def fillNum12(player_b):#the hole filled in 12 holes
    numHoleFill=0
    for i in range(2):
        if(player_b[i]!=0):
            numHoleFill+=1
...
#    counter+=1
    ...
    if(MAXSTEP==0):                                           #when we reach the end of the prediction 
        ...
        if(isPlayer):                                     #return the difference of score the bigger the better for us
            ...
            score += fillNum12(p_board)*12.5#15#3#3
            score += (3.75 if p_board[2]!=0 else 0)#tried 4.1667 and 2.5, 3.3333
            score += p_man*1.75#1
            score += sumPB#p_board[3]+p_board[4]+p_board[5]
            score += case_cap_biggest(p_board,op_board)
            
            score -= fillNum12(op_board)*12.5#15#3#3
            score -= (3.75 if op_board[2]!=0 else 0)#tried 4.1667 and 2.5, 3.3333
            score -= op_man*1.75#1
            score -= sumOB#op_board[3]+op_board[4]+op_board[5]#
