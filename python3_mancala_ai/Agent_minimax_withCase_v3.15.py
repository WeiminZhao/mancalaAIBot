...
def case_cap(player_b,oppo_b):#capture opponent(player have chance to cap oppo, return how many available)
    pos_num=0
    for i in range(6):
        if(player_b[i]!=0 and player_b[i]%13+i<6 and (player_b[player_b[i]%13+i]==0 or player_b[i]%13==0) and oppo_b[5-(player_b[i]%13+i)]!=0):
            pos_num +=1
    return pos_num
...
#    counter+=1
    ...
    if(MAXSTEP==0):                                           #when we reach the end of the prediction 
        score=0
        if(isPlayer):                                     #return the difference of score the bigger the better for us
            #score += case_cap(p_board,op_board)*(1 if sumOB+op_man<=24 else 2)#how many chance to cap enemy-opposite
            #if(sumBoards>=46):#>42
            #score += case_free(p_board)#-case_free(op_board)
            #score += diffMancala(p_man,op_man)#*player_id#mancala difference
            score += p_man
            score += sumPB #+ p_board[5]#*(player_id)#+player_id-1)#-sumOB#board
            #score += weightSum(p_board)/(3 +(0.5 if player_id==1 else 0))#the weighted sum is helping in certain situation
            score += fillNum(p_board)*2#(1+player_id)#filled hole number(player's)
            
            #score += (p_board[0]+p_board[1]+p_board[2])
            #score -= case_free(op_board)*(1 if sumOB+op_man<=24 else 2)
            score -= (op_board[0]+op_board[1]+op_board[2])
            score -= fillNum123(op_board)*3
            score -= op_man
            #score += p_board[5]+p_board[5]
            return score

        else:#since in this area the opponent is our player, we reverse the flavor
            #score += case_cap(op_board,p_board)*(1 if sumPB+p_man<=24 else 2)#how many chance to cap enemy-opposite
            #if(sumBoards>=46):#>42
            #score += case_free(op_board)#-case_free(p_board)
            #score += diffMancala(op_man,p_man)#*player_id#mancala difference
            score += op_man
            score += sumOB #+ op_board[5]#*(player_id)#+player_id-1)#-sumPB #board
            #score += weightSum(op_board)/(3 +(0.5 if player_id==1 else 0))#the weighted sum is helping in certain situation
            score += fillNum(op_board)*2#(1+player_id)#filled hole number

            #score += (op_board[0]+op_board[1]+op_board[2])
            #score -= case_free(p_board)*(1 if sumPB+p_man<=24 else 2)
            score -= (p_board[0]+p_board[1]+p_board[2])
            score -= fillNum123(p_board)*3
            score -= p_man
            #score += op_board[5]+op_board[5]
            return score
    ##########################################################################
    ####evaluation############################################################
    pick=0
